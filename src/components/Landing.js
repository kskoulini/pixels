import React, { useMemo } from 'react';                   // ⬅️ add useMemo
...
// accept items as an argument
function hasUnreadMessages(items) {
  const progress = getProgress() || {};

  // total across ALL items
  const totalAll = items.length;
  const seenAll = progress['all']?.length || 0;

  // console.log('seenAll < totalAll: ',seenAll < totalAll);
  if (seenAll < totalAll) return true;

  // per-category check
  const totalsByCat = items.reduce((acc, it) => {
    acc[it.category] = (acc[it.category] || 0) + 1;
    return acc;
  }, {});

  for (const [cat, total] of Object.entries(totalsByCat)) {
    const seen = progress[cat] || 0;
    if (seen < total) return true;
  }
...
function Landing() {
  ...
  const unread = useMemo(() => hasUnreadMessages(ITEMS), [ITEMS]);

  return (
    <RetroWindow title="home-page">
      <div className="landing-wrap page-container" style={{ position: 'relative' }}>
        <button
          className="pixel-button pixel-button-ghost"
          onClick={async () => {
            await refreshAll();                           // updates items in context
            window.dispatchEvent(new Event('pixels:comments-updated')); // optional: notify comments listeners
            bootstrapCommentsFromStatic(cats, true);      // keep your existing call if needed
          }}
          style={{
            padding: '1px 4px',
            fontSize: '11px',
        ...
        <div className="landing-inner">
          <div className={`landing-title${unread ? ' unread' : ''}`}>
            <div className="new-message-bubble">
              <img src={chatbubble} alt="" className="bubble-background" />
              <span className="bubble-text">New message available</span>
            </div>
            <svg className='sprite' width="27" height="33" viewBox="0 0 27 33" fill="none" xmlns="http://www.w3.org/2000/svg">
